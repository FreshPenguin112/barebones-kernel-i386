...
int elf_run(const char *filename)
{
    ...
    // Only support 64-bit, little-endian, executable
    if (ehdr->e_ident[4] != 2 || ehdr->e_ident[5] != 1 || ehdr->e_type != 2)
    {
        return -3;
    }
    ...
    for (int i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        if (ph->p_type != 1)
            continue; // PT_LOAD
        char *dest = (char *)(uintptr_t)ph->p_vaddr;
    ...
    entry_fn_t entry = (entry_fn_t)(uintptr_t)ehdr->e_entry;
