#include "vga_text_gfx.h"
#include <stddef.h>
#include <stdint.h>

// 8x8 font for ASCII 0x20-0x7F (space to DEL)
...
{0x00,0x00,0x1B,0x36,0x36,0x0E,0x06,0x00},
...
// You must provide this function for your kernel:
...
void vga_draw_char8x8(int x, int y, char c, uint8_t color) {
    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            // Fix: use (bits & (1 << col)) for correct bit order
            if (bits & (1 << col)) {
...
void vga_draw_string8x8(int x, int y, const char *s, uint8_t color) {
    for (int i = 0; s[i]; i++) {
        vga_draw_char8x8(x + i * 8, y, s[i], color);
    }
}
